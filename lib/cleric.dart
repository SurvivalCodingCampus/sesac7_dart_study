...
class Cleric {
  ...
  int pray(int second) {
    ...
    if (second > 0) { // 기도 시간이 양수일 때만 실질적인 마법 사용.
      final int mpRecoveryAmount = second + Random().nextInt(prayCorrectionRange); // mp 회복 전체양(초과분도 포함), 기도 시간 + 0~2 랜덤 정수

      if (mp + mpRecoveryAmount <= maxMp) { // 10까지는 mpRecoveryAmount 값이 그대로 회복량이 됨.
        mp += mpRecoveryAmount;
        result = mpRecoveryAmount;
      } else { // 회복했을 때 10을 넘어갈 경우
        final int mpBeforeRecovery = mp; // 회복 전 mp 값을 임시로 저장하고,
        mp = maxMp; // mp를 최대치까지 회복한 후,
        result = maxMp - mpBeforeRecovery; // mp 최대치(10) - 회복 전 mp 값(mpBeforeRecovery)을 반환.
      }
    }
